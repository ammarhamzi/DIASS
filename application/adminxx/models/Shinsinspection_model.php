...
class Shinsinspection_model extends CI_Model {
...
// get all
    public function get_all() {
        ...
        $this->db->from('shinspermit a');
        $this->db->join('permit p', 'a.shinspermit_permit_id = p.permit_id', 'inner' );
        $this->db->join('vehicle v', 'a.shinspermit_vehicle_id = v.vehicle_id', 'inner' );
    ...
    public function get_by_id($id) {
        $this->db->select('a.shinspermit_id shinspermit_id');
        $this->db->select('a.shinspermit_inspectionapproval inspectionapproval');
        $this->db->select('a.shinspermit_inspectionapproval_verification inspectionapproval_verification');
        $this->db->select('a.shinspermit_inspection_remark inspection_remark');
        $this->db->select('a.shinspermit_inspection_verification_remark inspection_verification_remark');
        $this->db->select('u.user_name inspector');
        $this->db->select('u2.user_name manager');        
        $this->db->select('a.shinspermit_managerverified_date managerverified_date');
        $this->db->select('a.shinspermit_inspection_date inspection_date');
        $this->db->select('p.permit_bookingid booking_id');
        $this->db->select('v.vehicle_registration_no vehicle_regno');
        $this->db->select('vc.vehiclegroup_name vehicle_type');
        $this->db->select('v.vehicle_year_manufacture');
        ...
        $this->db->join('vehiclegroup vc', 'v.vehicle_group = vc.vehiclegroup_id', 'inner' );
        $this->db->join('company c', 'v.vehicle_company_id = c.company_id', 'inner' );
        $this->db->join('userlist u', 'u.user_id = a.shinspermit_result_inspector_id', 'left' );
        $this->db->join('userlist u2', 'u2.user_id = a.shinspermit_managerverified_id', 'left' );
    ...
    // get data by id
    public function get_shinschecklist_by_id($id) {
        $this->db->select('*');
    ...
    public function listajax($columns, $start, $length, $filter="", $sort="", $sorttype=""){
        ...
        $this->db->order_by('shinspermit_id', 'DESC');
        }
    ...
    public function update_checklist($data, $id) {
        $this->db->update_batch('shinschecklist', $data, 'shinschecklist_id');
        return $this->get_shinschecklist_by_id($id);
    }

    public function update_inspection_result($data_id, $data_shinspermit, $data_permit, $data_permit_timeline) {
        $data_id = (object) $data_id;

        $this->db->where('shinspermit_id', $data_id->shinspermit_id);
        $this->db->update('shinspermit', $data_shinspermit);
