...
class Permitpendinginspectionmanager_model extends CI_Model
{
...
// delete data
    ...
    public function listajax($location,$columns, $start, $length, $filter = "", $sort = "", $sorttype = "")
    {
        $i = 0;
        $this->db->select('
    permit.*,
...
permit_type.permit_type_desc AS permit_type_desc,
    ...
    permit_officialstatus.permit_officialstatus_name AS permit_officialstatus_name_permit_officialstatus', false);
        $this->db->distinct();
        $this->db->where('permit_deleted_at');
        $this->db->where('permit_status', 'inspectionpassed');
        if($location!="all"){
    ...
    public function recordsFiltered($location,$columns, $filter = "")
    {
        $i = 0;
