...
class Permitpendingdocscheckvehicle_model extends CI_Model
{
...
CASE permit_typeid
    ...
    ELSE NULL
END as permit_inspection_date
    ', false);
        $this->db->distinct();
        $this->db->where('permit_deleted_at');
        $this->db->where('permit_officialstatus', 'pending');
        if($location!="all"){
        ...
        if ($sort != "") {
            ...
            $this->db->order_by('permit_inspection_date', 'ASC');
        }
