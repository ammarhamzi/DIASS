...
class Service_charges_model extends CI_Model
{
    ...
    public function get_all()
    {
        ...
        $this->db->order_by('servicecharges.created_at','desc');
        return $query = $this->db->get()->result();
    }

    public function pdf_read($id)
    {
        $this->db->select('servicecharges.*, charges_types.name as charges_types_name, (servicecharges.servicecharges_qty*charges_types.price_actual) as total_charges');
        $this->db->where('servicecharges.active', '1');
...
// delete data
    ...
    function category_charges_price_per_date($date,$category,$paymentMethod = 1,$shift = 4,$paymentLocation='')
    {
        ...
        $this->db->select("servicecharges.servicecharges_id, 
                            SUM(
                                CASE 
    ...
    function category_qty_per_date($date,$category,$paymentMethod = 1,$shift = 4,$paymentLocation='')
    {
        if(empty($date) || $date == '1970-01-01' || $date == '1900-01-01' || $date == '0000-00-00')
        {
            return 0;
