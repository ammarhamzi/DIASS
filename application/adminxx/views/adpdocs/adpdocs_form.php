...
<div class="container-fluid">
    ...
    <div class="panel panel-info">
        ...
        <div class="panel-body">
            ...
            <?php echo $this->lang->line('adpdocs_adppermit_id');?> <sup><span class="glyphicon glyphicon-star text-danger" aria-hidden="true"></span></sup> <?php echo form_error('adpdocs_adppermit_id') ?>
          </label>

                            <div class="col-md-9"><select class="form-control select2" name="adpdocs_adppermit_id" id="adpdocs_adppermit_id">
<option value="">-SELECT-</option>
<?php
foreach ($adppermit as $value) {
?>
<option value="<?php echo $value->adppermit_id;?>" <?php echo ($value->adppermit_id===$adpdocs_adppermit_id?'selected="selected"':"");?>><?php echo $value->adppermit_permit_id;?></option>
<?php
}
?>
</select>
                        ...
                        <div class="row">
                            <label class="col-md-3 text-right">
            <?php echo $this->lang->line('adpdocs_attendanceslip');?> <?php echo form_error('adpdocs_attendanceslip') ?>
          </label>
                            <div class="col-md-9">
                                <div class="upload-imagedocs-messages">
                                    <?php
if($adpdocs_attendanceslip){
?>
...
?>
                                </div><input type="hidden" class="form-control" id="adpdocs_attendanceslip" name="adpdocs_attendanceslip" value="<?php echo(isset($adpdocs_attendanceslip)?$adpdocs_attendanceslip:" ") ?>" readonly>
    ...
    <span class="btn btn-default btn-remote-file">Browse …
                                    ...
                                    <input type="text" class="form-control display_adpdocs_attendanceslip" value="<?php echo(isset($adpdocs_attendanceslip)?$adpdocs_attendanceslip:" ") ?>" readonly>
                                </div>
                                <div class="uploadrestrict">(supported png,jpg,jpeg,doc,docx,pdf,txt)</div>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-offset-3 col-md-9">
                                <input type="hidden" name="adpdocs_id" value="<?php echo (isset($id)?$id:" "); ?>" />
                                ...
                                <a href="<?php echo site_url('adpdocs') ?>" class="btn btn-default"><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span> <?php echo $this->lang->line('back');?></a>
                            </div>
                        </div>
                    </form>
                </div>
...
<?php echo form_open_multipart('upload/do_upload', array('class' => 'upload-imagedocs'));?>
    ...
    jQuery(document).ready(function($) {
        var options = {
            beforeSend: function() {
                ...
                $(".upload-imagedocs-messages").html('<p><img src = "<?php echo base_url() ?>resources/shared_img/system/ajax-loader.gif" class = "loader" /></p>');
            },
            complete: function(response) {
                ...
                console.log(arr);
                if (arr[1] == "success") {
                    $("#adpdocs_attendanceslip").val(arr[0]);
                    $(".upload-imagedocs-messages").html('<p><img src = "<?php echo base_url() ?>resources/shared_img/' + arr[0] + '" width="100" height="100" /></p>');
                } else {
                    $(".upload-imagedocs-messages").html('<p>' + arr[0] + '</p>');
                }
            }
        };
        // Submit the form
        $(".upload-imagedocs").ajaxForm(options);
        return false;
    });
</script>
<?php
        ...
        $(document).ready(function() {
            ...
            $(document).ready(function() {
                $('.btn-file :file').on('fileselect', function(event, numFiles, label) {
                    ...
                    if (input.length) {
                        input.val(log);
                        $(".display_adpdocs_attendanceslip").val(log);
