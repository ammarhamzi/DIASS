...
class Upload extends CI_Controller
{
    ...
    public function do_upload()
    {
        ...
        if ($this->input->post('submit')) {
            ...
            else if ($this->input->post('filetype') == "files") {
                $path = '../resources/shared_file/';
            } 
            else if ($this->input->post('filetype') == "files_sc") {
                $path = '../resources/shared_file/';
            } 
            else {
            ...
            if ($this->input->post('filetype') == "video") {
                ...
                if( isset($_FILES["uploadfile"]["name"]) && count($_FILES["uploadfile"]["name"]) > 1)
                {
                    $i = 0;
                    foreach($_FILES["uploadfile"]["name"] as $rn)
                    {
                        $raw_name = preg_replace('/\s+/', '_', $rn);
                        $name = shuffle_name($raw_name);
                        $_FILES["uploadfile"]["name"][$i] = $name;
                        $i++;
                    }
                }
                else
                ...
                if( isset($_FILES["uploadfile"]["name"]) && count($_FILES["uploadfile"]["name"]) > 1)
                {
                    ...
                    foreach($_FILES["uploadfile"]["name"] as $rn)
                    {
                        $raw_name = preg_replace('/\s+/', '_', $rn);
                        $name = shuffle_name($raw_name);
                        $_FILES["uploadfile"]["name"][$i] = 'sc_'.$name;
                        $i++;
                    }
                }
                else
