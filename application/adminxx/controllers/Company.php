...
//check if parentchild exist
        ...
        if ($this->permission->cp_create == true) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $data = [
                    ...
                    'company_lastchanged_by' => $this->session->userdata('id'),
    ...
    public function get_json()
    {
        ...
        foreach ($results as $r) {
            ...
            $exist = $this->pic_model->notexist_pic($r['company_id']);
            ...
            if ($this->permission->cp_delete == true && $exist == 0) {
                $rud .= anchor(site_url('company/delete_update/' . fixzy_encoder($r['company_id'])), '<button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></button>', 'onclick="javasciprt: return confirm(\'' . $this->lang->line('delete_alert') . '\')"');
            }
            array_push($data, [
                $i,
                $r['company_name'],
