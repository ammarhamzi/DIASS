...
class Wipbriefingpermit extends MY_Controller
{
    ...
    public function attendance()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $this->content = 'wipbriefingpermit/wipbriefingpermit_attendance';
            ##--slave_combine_to_list--##
            $this->layout($data, $setting);
...
//check if parentchild exist
    ...
    public function briefingattend($date)
    {
        ...
        if ($this->permission->cp_create == true) {
            ...
            $wipbriefing = $this->wipbriefingpermit_model->get_all_by_date($date);
            $data = [
                'button' => 'Create',
                'action' => site_url('wipbriefingpermit/check_action'),
                'wipbriefing' => $wipbriefing,
                'course_date' =>$date,

            ];
            $this->content = 'wipbriefingpermit/wipbriefingpermit_briefing';
            $this->layout($data, $setting);
    ...
    public function check_action()
    {
        ...
        $selected_wipbriefingpermit = $this->input->post("wipbriefingpermit_id");
        $nowdatetime   = date('Y-m-d H:i:s');
        //$permitid = $this->input->post("permit_id", true);

        foreach ($selected_wipbriefingpermit as $wipbriefingpermit) {
            $permit_ = explode("|", $wipbriefingpermit);
            $wipbriefingpermitid = $permit_[0]; // piece1
            $permitid = $permit_[1]; // piece2

                $data = [
                    'wipbriefingpermit_attendwipbriefing' => 'y',
                    'wipbriefingpermit_certbytrainer' => $this->input->post('wipbriefingpermit_certbytrainer', true),
                    'wipbriefingpermit_certbytrainer_date' => $this->input->post('wipbriefingpermit_certbytrainer_date', true),
                    'wipbriefingpermit_updated_at' => $nowdatetime,
                    'wipbriefingpermit_lastchanged_by' => $this->session->userdata('id'),
                ];
                $this->wipbriefingpermit_model->update($wipbriefingpermitid, $data);
                /* $this->logQueries($this->config->item('dblog')); */
                ...
                /* $this->logQueries($this->config->item('dblog')); */
                        ...
                        $data_timeline = [
                            ...
                            'permit_timeline_name' => ATTEND_BRIEFING_WIPBRIEFING,
                            'permit_timeline_desc' => ATTEND_BRIEFING_WIPBRIEFING_DESC,
