...
class Cspermit extends MY_Controller
{
    ...
    public function attendance()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $this->content = 'cspermit/cspermit_attendance';
            ##--slave_combine_to_list--##
            $this->layout($data, $setting);
...
//check if parentchild exist
    ...
    public function briefingattend($date)
    {
        ...
        if ($this->permission->cp_create == true) {
            ...
            $csbriefing = $this->cspermit_model->get_all_by_date($date);
            $data = [
                'button' => 'Create',
                'action' => site_url('cspermit/check_action'),
                'csbriefing' => $csbriefing,
                'course_date' =>$date,

            ];
            $this->content = 'cspermit/cspermit_briefing';
            $this->layout($data, $setting);
    ...
    public function check_action()
    {
        ...
        $selected_cspermit = $this->input->post("cspermit_id");
        $nowdatetime   = date('Y-m-d H:i:s');
        //$permitid = $this->input->post("permit_id", true);

        foreach ($selected_cspermit as $cspermit) {
            $permit_ = explode("|", $cspermit);
            $cspermitid = $permit_[0]; // piece1
            $permitid = $permit_[1]; // piece2

                $data = [
                    'cspermit_attendcsbriefing' => 'y',
                    'cspermit_certbytrainer' => $this->input->post('cspermit_certbytrainer', true),
                    'cspermit_certbytrainer_date' => $this->input->post('cspermit_certbytrainer_date', true),
                    'cspermit_updated_at' => $nowdatetime,
                    'cspermit_lastchanged_by' => $this->session->userdata('id'),
                ];
                $this->cspermit_model->update($cspermitid, $data);
                /* $this->logQueries($this->config->item('dblog')); */
                ...
                /* $this->logQueries($this->config->item('dblog')); */
                        ...
                        $data_timeline = [
                            ...
                            'permit_timeline_name' => ATTEND_BRIEFING_CS,
                            'permit_timeline_desc' => ATTEND_BRIEFING_CS_DESC,
                ...
                $this->session->set_flashdata('message', 'Update Record Success');
                redirect(site_url('cspermit/attendance'));
