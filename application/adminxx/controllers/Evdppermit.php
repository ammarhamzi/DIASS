...
/*        if ($this->permission->showlist == true) {
    ...
    public function attendance()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $this->content = 'evdppermit/evdppermit_attendance';
            ##--slave_combine_to_list--##
            $this->layout($data, $setting);
...
//check if parentchild exist
    ...
    public function briefingattend($date)
    {
        ...
        if ($this->permission->cp_create == true) {
            ...
            $data = [
                'button' => 'Create',
                'action' => site_url('evdppermit/check_action'),
                'evdpbriefing' => $evdpbriefing,
                'course_date' =>$date,

            ];
            $this->content = 'evdppermit/evdppermit_briefing';
            $this->layout($data, $setting);
...
public function briefingattend2($date,$loc)
    {
        ...
        if ($this->permission->cp_create == true) {
            ...
            //$evdpbriefing = $this->evdppermit_model->get_all_by_date2($date);
            //second modification
            $evdpbriefing = $this->evdppermit_model->get_all_by_date3($date,$loc);
            $data = [
                'button' => 'Create',
                'action' => site_url('evdppermit/check_action'),
                'evdpbriefing' => $evdpbriefing,
    ...
    public function check_action()
    {
        ...
        $selected_evdppermit = $this->input->post("evdppermit_id");
        $nowdatetime   = date('Y-m-d H:i:s');
        //$permitid = $this->input->post("permit_id", true);

        foreach ($selected_evdppermit as $evdppermit) {
            $permit_ = explode("|", $evdppermit);
            $evdppermitid = $permit_[0]; // piece1
            $permitid = $permit_[1]; // piece2

                $data = [
                    'evdppermit_attendterminalbriefing' => 'y',
                    'evdppermit_certbytrainer' => $this->input->post('evdppermit_certbytrainer', true),
                    'evdppermit_certbytrainer_date' => $this->input->post('evdppermit_certbytrainer_date', true),
                    'evdppermit_updated_at' => $nowdatetime,
                    'evdppermit_lastchanged_by' => $this->session->userdata('id'),
                ];
                $this->evdppermit_model->update($evdppermitid, $data);
                /* $this->logQueries($this->config->item('dblog')); */
                ...
                /* $this->logQueries($this->config->item('dblog')); */
                        ...
                        $data_timeline = [
                            ...
                            'permit_timeline_name' => ATTEND_BRIEFING_EVDP,
                            'permit_timeline_desc' => ATTEND_BRIEFING_EVDP_DESC,
                ...
                $this->session->set_flashdata('message', 'Update Record Success');
                redirect(site_url('evdppermit/attendance'));
