...
/*                      if($this->permission->cp_read == true){
    ...
    {
        $row = $this->avpevpinspectionmanagement_model->get_slot($type);
/*    var_dump($row);
exit;*/
        foreach ($row as $value) {
            $count_slottaken = $this->avpevpinspectionmanagement_model->get_slottaken_avp($value->avpevpinspectionmanagement_date);
            $count_slottaken2 = $this->avpevpinspectionmanagement_model->get_slottaken_evp($value->avpevpinspectionmanagement_date);
            $total_slottaken = $count_slottaken + $count_slottaken2;
/*          $available  = (int) $value->avpevpinspectionmanagement_slot - (int) $value->avpevpinspectionmanagement_slottaken;
            $title = $value->avpevpinspectionmanagement_location.'-'.$available.'/'. $value->avpevpinspectionmanagement_slot;*/
            $title = $value->avpevpinspectionmanagement_location.'- Booked '.(!empty($total_slottaken)?$total_slottaken:'0').'/'. $value->avpevpinspectionmanagement_slot;
