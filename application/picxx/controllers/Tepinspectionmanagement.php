...
/* $this->logQueries($this->config->item('dblog')); */
  ...
  public function _rules() {
    ...
    $lastdate = date('Y-m-d', strtotime('+2 months'));
        $row = $this->tepinspectionmanagement_model->get_slot($firstdate, $lastdate, $type);
/*    var_dump($row);
exit;*/
        foreach ($row as $value) {
        $count_slottaken = $this->tepinspectionmanagement_model->get_slottaken($value->tepinspectionmanagement_date,$value->tepinspectionmanagement_location);
...
$title = $value->tepinspectionmanagement_location.'-'.$count_slottaken.'/'. $value->tepinspectionmanagement_slot;
