...
<script src="<?php echo base_url('resources/shared_js/select2/dist/js/select2.min.js'); ?>" crossorigin="anonymous"></script>
  ...
  <div class="form-group">
    <label for="avp_insurancedoc">Insurance coverage <sup><span class="glyphicon glyphicon-star text-danger" aria-hidden="true"></span></sup> <?php echo form_error('avp_insurancedoc') ?><span class="text-danger" id="avp_insurancedoc_error"></span></label>

                                <iframe frameBorder="0" width="100%" height="250px" src="/Uploadfiles/avp_insurancedoc/<?php echo $vehicle_id;?>"></iframe>
  </div>
...
<!--                                                    <p>
<p>Selected Inspection Date:<span style="font-size: 10pt"><sup><span class="glyphicon glyphicon-star text-danger" aria-hidden="true"></span></sup></span> </p><h3><span class="label label-warning" id="inspection_date_display"></span></h3> <?php echo form_error('inspection_date') ?><span class="text-danger" id="inspection_date_error"></span>
                                                    </p><br>-->
...
<div class="form-group">
    ...
    <select id="location" name="location" class="form-control">
        ...
        <option value="Airside KLIA2">Airside KLIA2</option>
                        ...
                        <div class="row">
                            <div class="col-md-12">
<p>Please upload the following documents <sup><span class="glyphicon glyphicon-star text-danger" aria-hidden="true"></span></sup> <?php echo form_error('avp_requireddoc') ?><span class="text-danger" id="avp_requireddoc_error"></span></p>
<ul>
    <li>Letter of employer/owner</li>
    <li>Letter of award/contract [optional]</li>
    ...
    <li>Perakuan kelayakan mesin angkat (PMA) [optional]</li>
                                ...
                                <iframe frameBorder="0" width="100%" height="250px" src="/Uploadfiles/avp_requireddoc/<?php echo $vehicle_id;?>"></iframe>
                            </div>
...
<!--<td><?php echo $mtwchecklist->mtwchecklist_desc ?></td>-->
                                ...
                                <td style="text-align:center; white-space: nowrap;">
                                    ...
                                    $isrequired = '';
                                    //green
                                    if($avpcategory=='green'){
                                       if($mtwchecklist->mtwchecklist_name=="Call Sign"){
                $isrequired = 'checked onclick="this.checked=!this.checked;"';
}
                                    //red
                                    }elseif($avpcategory=='red'){
                                       if($mtwchecklist->mtwchecklist_name=="Call Sign" || $mtwchecklist->mtwchecklist_name =="VHF & Transponder"){
                $isrequired = 'checked onclick="this.checked=!this.checked;"';
}
                                    }else{
                $isrequired = '';
                                    }
                                        echo '<input ' .$isrequired. ' id="mtwchecklist_selected" name="mtwchecklist_selected[]" type="checkbox" value="' .$mtwchecklist->mtwchecklist_id. '">';
                                    ?>
                                    </div>
                                </td>
                  </tr>
...
<!--<td><?php echo $mtwchecklist->mtwchecklist_desc ?></td>-->
                ...
                <input id="avp_requireddoc" type="hidden" name="avp_requireddoc" value="">
                <input id="avp_insurancedoc" type="hidden" name="avp_insurancedoc" value="">
...
<input id="avpcategory" type="hidden" name="avpcategory" value="<?php echo $avpcategory;?>">
...
<script>
    ...
    function getuploadfiles(processtype, data) {
        //alert(data);
        if (processtype == 'avp_requireddoc') {
            $("#avp_requireddoc").val(data);
        }
         if(processtype=='avp_insurancedoc'){
         $("#avp_insurancedoc").val(data);
         }
    ...
    $(document).ready(function() {
        $(function() {
            ...
            $('#calendar').fullCalendar({
                ...
                defaultView: 'month',
                events: {
                    url: '/Avpevpinspectionmanagement/get_availableslot/avp',
                    type: 'POST',
                    data: {},
                    error: function() {
...
if($("#avp_insurancedoc").val() == "") {
    $("#avp_insurancedoc_error").html('<span class="alert_custom">Required</span>') ;
    status = 1;
}else{
$("#avp_insurancedoc_error").html('') ;
}
/*if($("#driver_photo").val() == "") {
...
if($("#avp_requireddoc").val() == "") {
    $("#avp_requireddoc_error").html('<span class="alert_custom">Required</span>') ;
    status = 1;
}else{
    var words = $('#avp_requireddoc').val().split(',');
    //alert(words.length);
    if(words.length < 2){
     $("#avp_requireddoc_error").html('<span class="alert_custom">Some required file(s) missing</span>') ;
     status = 1;
    }else{
$("#avp_requireddoc_error").html('') ;
}
}
...
if($("input[name='fireext_require']:checked").val() == null) {
    $("#fireext_require_error").html('<span class="alert_custom">Required</span>') ;
    status = 1;
}else{
$("#fireext_require_error").html('') ;
}

if($("input[name='fireext_require']:checked").val() == 'y') {
