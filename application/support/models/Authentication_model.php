...
class Authentication_model extends CI_Model
{
    ...
    public function get_login_username($username)
    {
        ...
        $this->db->where('user_deleted_at');
        $this->db->where('user_username', $username);
        $this->db->where(" (userlist.user_customid > 0) ");
        $this->db->from('userlist');
        $query = $this->db->get();
        if ($query->num_rows() >= 1) {
            return $query->row();
    ...
    public function get_login_pic($username)
    {
        $this->db->select('userlist.*');
        $this->db->from('userlist');
        $this->db->join('pic','pic.pic_id = userlist.user_customid');
        $this->db->where('userlist.user_isactive', '1');
        $this->db->where('userlist.user_deleted_at IS NULL');
