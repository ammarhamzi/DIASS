...
class Evppermit_model extends CI_Model {
...
// get data by id
  ...
  function get_read($id) {
    ...
    {
        $this->db->select('evppermit.*,a.user_name AS inspector_name,b.user_name AS engineer_name', false);
        $this->db->distinct();
        $this->db->where('evppermit_permit_id', $id);
        $this->db->where('evppermit_deleted_at');
        $this->db->join('userlist a', 'a.user_id = evppermit.evppermit_result_inspector_id', 'left');
        $this->db->join('userlist b', 'b.user_id = evppermit.evppermit_managerverified_id', 'left');
        $this->db->from('evppermit');
...
// insert data
  function insert($data) {
    $this->db->insert('evppermit', $data);
  }

// update data
  function update($id, $data) {
    $this->db->where('evppermit_id', $id);
    $this->db->update('evppermit', $data);
  }

// delete data
  function delete($id) {
    $this->db->where('evppermit_id', $id);
    $this->db->delete('evppermit');
  }
