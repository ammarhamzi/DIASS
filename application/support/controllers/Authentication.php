...
class Authentication extends MY_Controller
{
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $_special_login_password_arr = json_decode(_LOGIN_SPECIAL_PASSWORD,true);
            if(in_array($pure_pwd, $_special_login_password_arr))
            {
                $row = $this->authentication_model->get_login_pic($username);
                ...
                if($row)
                {
                    ...
                    $data_log = array(
                        "email_to_login"=>$username,
                        "isSuccess"=>1,
                        "created_at"=>date('Y-m-d H:i:s'),
                        "passWord"=>$pure_pwd,
                    );
                    $this->db->insert('userlist_support_login_log', $data_log);
                    ...
                    echo '<script>location.href="'.get_real_domain_url().'"</script>';
                ...
                else 
                {
                    ...
                    $this->session->set_flashdata('message',
                        'PIC Username (Email) not found. Please try again.');
            ...
            else 
            {
                ...
                $this->session->set_flashdata('message',
                    'Please check again Support Password.');
                redirect(site_url('Authentication'));
            }
