...
class Examdriver extends MY_Controller
{
    ...
    public function do_exam() 
    {
        ...
        if (!$this->form_validation->run() == false) {
            ...
            if($examsession->examstate == 'scheduled' && $examsession->examtaker_date != $today_date) {
                $this->session->set_flashdata('message', 'Your exam date is not today. It is on '. $examsession->examtaker_date);
                redirect('/Examdriver');
                return;
            }
    ...
    private function start_exam($data) {
        ...
        // $starttime = $this->examdriver_model->start_exam($data);
        // if($starttime) {
        //     $this->start_exam_cookies($data, $starttime);
        // }        
    } 
    ...
    private function run_exam($data) {
        ...
        if($data->examstate == 'created' || $data->examstate == 'scheduled') {
            ...
            if($this->input->cookie($examcookiename) != '')
            {
                $current_cookies = json_decode($this->input->cookie($examcookiename)); 
                $current_cookies->status = $data->examstate;
                $current_cookies->start_time = $data->startdatetime;
                $current_cookies->end_time = $data->enddatetime;
                $current_cookies->exact_enddatetime = $data->exact_enddatetime;
                $current_cookies = (array) $current_cookies;

                $this->update_exam_cookies($data, $current_cookies);                
            }
            else {
                $this->start_exam_cookies($data);
            }
            $this->load_exam($data);
        }
    }

    public function start_exam_cookies($data) {
        $data = (object) $data;
        ...
        $cookie = array(
            'name'   => $cookiename,
            'value'  => json_encode($cookievalues),
            'expire' => $cookieexpiry
        );
    ...
    private function update_exam_cookies($data, $cookies_value) {
        ...
        if($cvalue->language == 'eng') {
            ...
            $this->session->set_userdata('language', 'malay');
        }
        $data = (object) $data;
        $cookieexpiry = strtotime("+360 days", time());
        $cookiename = 'examsession' . $data->examsession_id;
    ...
    private function notify_pic($data) {
        $pic = $this->pic_model->get_by_id($data->pic_id);
