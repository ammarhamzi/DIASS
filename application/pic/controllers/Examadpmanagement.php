...
/* $this->logQueries($this->config->item('dblog')); */
  ...
  public function _rules() {
    ...
    $firstdate = addWorkingDays($newdate,'3')->format('Y-m-d');
        ...
        $row = $this->examadpmanagement_model->get_slot($firstdate, $condition,$drivingclass);
/*    var_dump($row);
exit;*/
        foreach ($row as $value) {
        $count_slottaken = $this->examadpmanagement_model->get_slottaken($value->exammanagement_date,$value->exammanagement_location,$value->exammanagement_category);
...
$title = $value->exammanagement_location.'-(' .$value->exammanagement_category. ')-'.$count_slottaken.'/'. $value->exammanagement_slot;

if($value->exammanagement_location == 'KLIA'){
$color = '#C0392B';
}elseif($value->exammanagement_location == 'KLIA2'){
$color = '#6C3483';
}
            $calendar[] = [
                ...
                "color"  => $color,
                "className" => $value->exammanagement_category,
                "taken" => $count_slottaken,
                "available" => $value->exammanagement_slot,
            ];
        }
