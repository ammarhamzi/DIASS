...
/* $this->logQueries($this->config->item('dblog')); */
  ...
  public function _rules() {
    ...
    $firstdate = addWorkingDays($newdate,'3')->format('Y-m-d');
        ...
        $row = $this->evdpbriefingmanagement_model->get_slot($firstdate);
/*    var_dump($row);
exit;*/
        foreach ($row as $value) {

$title = $value->evdpbriefingmanagement_location.'-(' .$value->evdpbriefingmanagement_category. ')-'.(!empty($value->evdpbriefingmanagement_slottaken)?$value->evdpbriefingmanagement_slottaken:'0').'/'. $value->evdpbriefingmanagement_slot;
