...
/* $this->logQueries($this->config->item('dblog')); */
  ...
  public function _rules() {
    ...
    public function get_availableslot($classtype)
    {
        $row = $this->examonlymanagement_model->get_slot($classtype);
/*    var_dump($row);
exit;*/
        foreach ($row as $value) {
            $available  = (int) $value->examonlymanagement_slot - (int) $value->examonlymanagement_slottaken;
            if($value->examonlymanagement_session=='m'){
            $desc = 'Morning - '.$available.'/'. $value->examonlymanagement_slot;
            }elseif($value->examonlymanagement_session=='a'){
            $desc = 'Afternoon - '.$available.'/'. $value->examonlymanagement_slot;
            }
            $calendar[] = [
                "id" => $value->examonlymanagement_id,
                "title" => $desc,
                "start" => $value->examonlymanagement_date,
                "end" => $value->examonlymanagement_date,
                "className" => $value->examonlymanagement_session,
            ];
        }
