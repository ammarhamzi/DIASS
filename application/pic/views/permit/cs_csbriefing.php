...
<div class="col-md-12">
  ...
  <div class="form-group">
    <label for="csbriefing_date">Briefing Date: <sup><span class="glyphicon glyphicon-star text-danger" aria-hidden="true"></span></sup> <?php echo form_error('csbriefing_date') ?><span class="text-danger" id="csbriefing_date_error"></span></label>
    <input type="text" class="form-control datepicker_local_insurancedate" id="csbriefing_date" name="csbriefing_date" placeholder="briefing/send vehicle date">
  </div>

  <div class="form-group">
    <label for="csbriefing_location">Operation Location: <sup><span class="glyphicon glyphicon-star text-danger" aria-hidden="true"></span></sup> <?php echo form_error('csbriefing_location') ?><span class="text-danger" id="csbriefing_location_error"></span></label>

    <select class="form-control" id="csbriefing_location" name="csbriefing_location">
...
<!--    <li>Previous Vehicle Service Sheet or PUSPAKOM Cert</li>
    ...
    <li>Offer letter (Proof of having job at KLIA/KLIA2)</li>
</ul>
...
$("#csbriefing_date_error").html('') ;
}

if($("#csbriefing_location").val() == "") {
   $("#csbriefing_location_error").html('<span class="alert_custom">Required</span>') ;
   status = 1;
}else{
$("#csbriefing_location_error").html('') ;
}
...
$("#cs_insurancedoc_error").html('') ;
}
if($("#cs_requireddoc").val() == "") {
   $("#cs_requireddoc_error").html('<span class="alert_custom">Required</span>') ;
   status = 1;
}else{
    var words = $('#cs_requireddoc').val().split(',');
    //alert(words.length);
    if(words.length < 3){
     $("#cs_requireddoc_error").html('<span class="alert_custom">Some required file(s) missing</span>') ;
     status = 1;
    }else{
$("#cs_requireddoc_error").html('') ;
}
}
