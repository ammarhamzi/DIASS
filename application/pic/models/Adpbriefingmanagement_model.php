...
// delete data
  ...
  function recordsFiltered($columns, $filter=""){
    ...
    public function get_slottaken($date,$location,$condition,$drivingclass)
    {
        $this->db->select("*");
        $this->db->where("adppermit_course_date",$date);
        $this->db->where("adppermit_verifybymahb_drivingarea",$drivingclass);
        $this->db->where("adppermit_course_location",$location);
        $this->db->where("permit_condition",$condition);
        $this->db->where("permit_officialstatus !=",'canceled');
        $this->db->where("permit_officialstatus !=",'rejected');
        $this->db->from("adppermit");
        $this->db->join('permit', 'permit.permit_id = adppermit.adppermit_permit_id');
        $query = $this->db->get();
