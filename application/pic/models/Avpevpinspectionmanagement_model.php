...
// get all
  function get_all() {
    $this->db->select('*,
	case avpevpinspectionmanagement_location
	when "KLIA" then "KLIA"
	when "KLIA2" then "KLIA2"
	end as avpevpinspectionmanagement_location', FALSE);
...
// delete data
  ...
  function recordsFiltered($columns, $filter=""){
    ...
    public function get_slot($firstdate = '',$lastdate = '',$type = '')
    {
        ...
        $this->db->where('avpevpinspectionmanagement_deleted_at');
        if($firstdate!=''){
        $this->db->where('avpevpinspectionmanagement_date >',$firstdate);
        }
        if($lastdate!=''){
        $this->db->where('avpevpinspectionmanagement_date <',$lastdate);
        }
        if($type!=''){
    ...
    public function get_slottaken($date)
    {
    $countotal= 0;
        $this->db->select("*");
