...
class Permitenforcementreport extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('permitenforcementreport_model');
    ...
    public function index()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $data = [
                ...
                'controller' => 'permitenforcementreport',
            ...
            $this->content = 'permitenforcementreport/permitall_list_process';
...
/*    print_r($_POST);
    exit;*/
        ...
        $columnfilter = [
            ...
            'offence_status' => $this->input->get('offence_status'),
            'company_name' => $this->input->get('company_name'),
            'driver_ic' => $this->input->get('driver_ic'),
            'vehicle_registration_no' => $this->input->get('driver_ic'),
        ...
        if($this->input->get('permit_location') == 'icno'){
        $results = $this->permitenforcementreport_model->listajax(
            $columnfilter,
...
//            $this->input->get('order')[0]['dir']
        );}else{$results = $this->permitenforcementreport_model->listajax2(
            $columnfilter,
            $this->input->get('start'),
            $this->input->get('length'),
            $this->input->get('search')['value']
...
//                $rud .= anchor(site_url('permitenforcementreport/' . strtolower($r['driver_name']) . '/' . fixzy_encoder($r['driver_id'])), '<button type="button" class="btn btn-info"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></button>') .
//                    ' ';
//            }
//$driver_id = "";
//                $val       = "";
...
//            }
if($this->input->get('permit_location') == 'icno'){
            $date = date_create($r['enforcements_date']);
            $date_closecancel = "";
            if($r['enforcements_main_cancel_date'] != null)
            {
                $date_closecancel = date_create($r['enforcements_main_cancel_date']);
                $date_closecancel = date_format($date_closecancel,'d-m-Y h:i A');
            }
            array_push($data, [
                $i,
                date_format($date,'d-m-Y'),
                $r['enforcements_time'],
                $r['enforcements_main_status'],
                $date_closecancel,
                $r['driver_name'],
                $r['driver_ic'],
                $r['company_name'],
 ...
 else {$date = date_create($r['enforcements_date']);
            array_push($data, [
                $i,
                date_format($date,'d-m-Y'),
                $r['enforcements_time'],
