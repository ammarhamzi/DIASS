...
//                    'vehicle_distance_color' => $distance_color,
    ...
    public function update($id)
    {
        ...
        if ($this->permission->cp_update == true) {
            ...
            $vehiclehistory = $this->vehicle_model->get_all_companyhistory(fixzy_decoder($id));
            /* $this->logQueries($this->config->item('dblog')); */
            if ($row) {
                $permitvehicle = $this->vehicle_model->get_by_vehicle_id(fixzy_decoder($id),trim($row->vehicle_type));
//                $tempstatus='';
//                if ($permitvehicle) {
//                    $tempstatus=$permitvehicle->permit_officialstatus;
//                }
                if ($permitvehicle != null)
                {
                $ph = $permitvehicle[0];
                }
                $vh = $vehiclehistory;
                $data = [
                    ...
                    'activepermitno' => ($permitvehicle != null)?$ph->permit_issuance_serialno:"",
                    'permitstatus' => ($permitvehicle != null)?$ph->permit_status:"",
                    'permitstatusdesc' => ($permitvehicle != null)?$ph->permit_status_desc:"",
...
/*             if($this->input->post('vehicle_type', true)=='AV'){
            ...
            'vehiclegroup_name'=>strtoupper($this->input->post('other_equipment', true)),
             ...
             $vehicle_group = $this->db->insert_id();
                ...
                $newcompany = $this->input->post('vehicle_company_id_dummy', true);
                if($newcompany == null)
                    $newcompany = $this->input->post('vehicle_company_id', true);

                $now  = date('Y-m-d H:i:s');
                $data = [
                    'vehicle_company_id' => $newcompany,
                    'vehicle_registration_no' => strtoupper($this->input->post('vehicle_registration_no', true)),
                    'vehicle_type' => $this->input->post('vehicle_type', true),
                    'vehicle_group' => $vehicle_group,
/*                    'vehicle_insurance_policy_no' => $this->input->post('vehicle_insurance_policy_no', true),
...
/*                    'vehicle_distance_color' => $distance_color,*/
                ...
                if ($newcompany != $this->input->post('vehicle_company_id_ori', true)) {
                    $datahistory = [
                        'vehiclecompanyhistory_vehicle_id' => fixzy_decoder($this->input->post('vehicle_id')),
                        'vehiclecompanyhistory_company_id' => $this->input->post('vehicle_company_id_ori', true),
                        'vehiclecompanyhistory_created_at' => $now,
                        'vehiclecompanyhistory_lastchanged_by' => $this->session->userdata('id'),
                    ];

                    $this->vehicle_model->createhistory($datahistory);
...
' ';
            ...
            if ($this->permission->cp_update == true) {
                $rud .= anchor(site_url('admin/vehicle/update/' . fixzy_encoder($r['vehicle_id'])), '<button type="button" class="btn btn-default"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>') .
                    ' ';
            }
/*            if ($this->permission->cp_update == true) {
...
$can_delete = '<a href="/admin/vehicle/delete/' . fixzy_encoder($r['vehicle_id']) . '" onclick="javasciprt:return confirm(\'' .$this->lang->line('delete_alert'). '\')"><button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></button></a>';
            ...
            array_push($data, [
                $i,
                '<a href="/admin/company/read/' . fixzy_encoder($r['company_id']) . '">'.$r['company_name_vehicle_company_id'].'</a>',
                '<a href="/admin/vehicle/show/'.fixzy_encoder($r['vehicle_id']).'">'.$r['vehicle_registration_no'].'</a>',
                $r['vehicle_type'],
                $r['vehicle_name'],
...
$r['vehicle_engine_capacity'],*/
                ...
                '<a href="/admin/vehicle/update/' . fixzy_encoder($r['vehicle_id']) . '"><button type="button" class="btn btn-default"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button></a> '.$can_delete,
...
/*                $rud  */
        ...
        echo json_encode(
            [
                ...
                "recordsFiltered" => $this->vehicle_model->recordsFiltered($columns,
