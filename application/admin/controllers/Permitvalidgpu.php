...
class Permitvalidgpu extends MY_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('permitvalidgpu_model');
        ...
        $this->lang->load('permitvalidgpu_lang', $this->session->userdata('language'));
    ...
    public function index()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $data = [
                ...
                'controller' => 'Permitvalidgpu',
                'pagetitle' => 'Active FFOP (Ground Power Unit) Permits',
            ];
    ...
    public function get_json($location='all')
    {
        ...
        $results = $this->permitvalidgpu_model->listajax(
            $location,
            $columns,
           $columnfilter,
        ...
        foreach ($results as $r) {
            ...
            if($r['permit_officialstatus_name_permit_officialstatus']=="completed"){
                $rud .= anchor(site_url('permitvalidgpu/termination/' . fixzy_encoder($r['permit_id'])), 'Terminate', 'class="btn btn-primary btn-xs"') .' '.anchor(site_url('permitvalidgpu/replacement/' . fixzy_encoder($r['permit_id'])), 'Replace', 'class="btn btn-primary btn-xs"') .' '. anchor(site_url('permitvalidgpu/' . strtolower($r['permit_type_name_permit_typeid']) . '/' . fixzy_encoder($r['permit_id'])), 'Renew', 'class="btn btn-primary btn-xs"').
                    ' ';
            }else{
            if($r['permit_officialstatus_name_permit_officialstatus']!="canceled" && $r['permit_officialstatus_name_permit_officialstatus']!="paid" && $r['permit_status']!="permitterminationpending" && $r['permit_status']!="permitreplacementpending"){
                $rud .= anchor(site_url('permitvalidgpu/cancellation/'  . fixzy_encoder($r['permit_id'])), 'Cancel', 'class="btn btn-primary btn-xs"') .
                    ' ';
            }
...
if($r['permit_type_name_permit_typeid']=="gpu"){
...
/*                $i, */
        ...
        echo json_encode(
            [
                ...
                "recordsTotal" => $this->permitvalidgpu_model->recordsTotal($location)->recordstotal,
                "recordsFiltered" => $this->permitvalidgpu_model->recordsTotal($location)->recordstotal,
                'data' => $data
            ]
        );
    }
...
;
/* End of file Permitvalidgpu.php */
/* Location: ./application/controllers/Permitvalidgpu.php */
