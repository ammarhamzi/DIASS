...
class Pbbpermit extends MY_Controller
{
    ...
    public function attendance()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $this->content = 'pbbpermit/pbbpermit_attendance';
            ##--slave_combine_to_list--##
            $this->layout($data, $setting);
...
//check if parentchild exist
    ...
    public function briefingattend($date)
    {
        ...
        if ($this->permission->cp_create == true) {
            ...
            $pbbbriefing = $this->pbbpermit_model->get_all_by_date($date);
            $data        = [
                'button' => 'Create',
                'action' => site_url('pbbpermit/check_action'),
                'pbbbriefing' => $pbbbriefing,
                'course_date' => $date,

            ];
            $this->content = 'pbbpermit/pbbpermit_briefing';
            $this->layout($data, $setting);
    ...
    public function assessment_status($date)
    {
    ...
    public function check_action()
    {
        ...
        if ($this->permission->cp_update == true) {
            ...
            $pbbpermit_id = $this->input->post("pbbpermit_id");

            $nowdatetime = date('Y-m-d H:i:s');
            //$permitid = $this->input->post("permit_id", true);

            foreach ($pbbpermit_id as $pbbpermit) {

                if ($this->input->post("pbbpermit_" . $pbbpermit)) {
                    $pbbpermit_toprocess = $this->input->post("pbbpermit_" . $pbbpermit);
                    $permit_             = explode("|", $pbbpermit_toprocess);
                    $pbbpermitid         = $permit_[0]; // piece1
                    $permitid            = $permit_[1]; // piece2
                    $status              = $permit_[2]; // piece2

                    $data = [
                        'pbbpermit_attendpbbbriefing' => $status,
                        'pbbpermit_certbytrainer' => $this->input->post('pbbpermit_certbytrainer', true),
                        'pbbpermit_certbytrainer_date' => $this->input->post('pbbpermit_certbytrainer_date', true),
                        'pbbpermit_updated_at' => $nowdatetime,
                        'pbbpermit_lastchanged_by' => $this->session->userdata('id'),
                    ];
                    $this->pbbpermit_model->update($pbbpermitid, $data);
                    /* $this->logQueries($this->config->item('dblog')); */
                    ...
                    if ($status == 'y') {
                        ...
                        $data_timeline = [
                            ...
                            'permit_timeline_name' => ATTEND_BRIEFING_PBB,
                            'permit_timeline_desc' => ATTEND_BRIEFING_PBB_DESC,
                        ...
                        $data_permit = [
                            'permit_status' => 'assessmentfailed',
                        ...
                        $data_timeline = [
                            'permit_timeline_permitid' => $permitid,
                            'permit_timeline_userid' => $this->session->userdata('id'),
                            'permit_timeline_name' => ATTEND_BRIEFING_PBB_FAIL,
                            'permit_timeline_desc' => ATTEND_BRIEFING_PBB_FAIL_DESC,
                            'permit_timeline_status' => 'assessmentfailed',
