...
class Permittimeline extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $permittimeline = $this->permittimeline_model->get_all();
            /* $this->logQueries($this->config->item('dblog')); */
            $data = [
                'permittimeline_data' => $permittimeline,
                'permission' => $this->permission,
            ];

            $this->content = 'permittimeline/permittimeline_list';
    ...
    public function read($id, $type = "normal")
    {
        ...
        if ($this->permission->cp_read == true) {
            ...
            $row = $this->permittimeline_model->get_read($id);
            /* $this->logQueries($this->config->item('dblog')); */
            if ($row) {
                $data = [
                    'permit_timeline_permitid' => $row->permit_timeline_permitid,
                    'permit_timeline_userid' => $row->permit_timeline_userid,
                    'permit_timeline_name' => $row->permit_timeline_name,
                    'permit_timeline_desc' => $row->permit_timeline_desc,
                    'permit_timeline_status' => $row->permit_timeline_status,
                    'permit_timeline_officialstatus' => $row->permit_timeline_officialstatus,
                    'permit_timeline_created_at' => $row->permit_timeline_created_at,
                    'permit_timeline_updated_at' => $row->permit_timeline_updated_at,
                    'permit_timeline_deleted_at' => $row->permit_timeline_deleted_at,
...
//check if parentchild exist
                    ...
                    $this->content = 'permittimeline/permittimeline_read';
                    ##--slave_combine_to_read--##
                    $this->layout($data, $setting);
                } else {
                    echo $this->load->view('permittimeline/permittimeline_read_raw', $data, true);
                }

            } else {
                $this->session->set_flashdata('message', 'Record Not Found');
                redirect(site_url('permittimeline'));
            }
    ...
    public function create()
    {
        ...
        if ($this->permission->cp_create == true) {
            ...
            $data = [
                'button' => 'Create',
                'action' => site_url('permittimeline/create_action'),
                ...
                'permit_timeline_updated_at' => set_value('permit_timeline_updated_at'),
                'permit_timeline_deleted_at' => set_value('permit_timeline_deleted_at'),
                'permit_timeline_lastchanged_by' => set_value('permit_timeline_lastchanged_by'),

            ];
            $this->content = 'permittimeline/permittimeline_form';
            $this->layout($data, $setting);
        ...
        if ($this->permission->cp_create == true) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $data = [
                    ...
                    'permit_timeline_updated_at' => $this->input->post('permit_timeline_updated_at', true),
                    'permit_timeline_deleted_at' => $this->input->post('permit_timeline_deleted_at', true),
                ...
                $this->permittimeline_model->insert($data);
    ...
    public function update($id)
    {
        ...
        if ($this->permission->cp_update == true) {
            ...
            $row = $this->permittimeline_model->get_by_id(fixzy_decoder($id));
            /* $this->logQueries($this->config->item('dblog')); */
            if ($row) {
                $data = [
                    'button' => $this->lang->line('edit'),
                    'action' => site_url('permittimeline/update_action'),
                    'id' => $id,
                    ...
                    'permit_timeline_updated_at' => set_value('permit_timeline_updated_at', $row->permit_timeline_updated_at),
                    'permit_timeline_deleted_at' => set_value('permit_timeline_deleted_at', $row->permit_timeline_deleted_at),
    ...
    public function update_action()
    {
        ...
        if ($this->permission->cp_update == true) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->permittimeline_model->update(fixzy_decoder($this->input->post('permit_timeline_id')), $data);
                /* $this->logQueries($this->config->item('dblog')); */
    ...
    public function delete($id)
    {
        ...
        if ($this->permission->cp_delete == true) {
            ...
            $id  = fixzy_decoder($id);
            $row = $this->permittimeline_model->get_by_id($id);
            /* $this->logQueries($this->config->item('dblog')); */
            if ($row) {
                $this->permittimeline_model->delete($id);
                /* $this->logQueries($this->config->item('dblog')); */
    ...
    public function delete_update($id)
    {
        ...
        if ($this->permission->cp_delete == true) {
            ...
            if ($row) {
                ...
                $this->permittimeline_model->update($id, $data);
                /* $this->logQueries($this->config->item('dblog')); */
                $this->session->set_flashdata('message', 'Delete Record Success');
                redirect(site_url('permittimeline'));
            } else {
    ...
    public function _rules()
    {
        ...
        $this->form_validation->set_rules('permit_timeline_updated_at', ' ', 'trim');
        $this->form_validation->set_rules('permit_timeline_deleted_at', ' ', 'trim');
    ...
    public function get_json()
    {
        ...
        $columns = [
            ...
            'permit_timeline_updated_at',
            'permit_timeline_deleted_at',
            'permit_timeline_lastchanged_by',

        ];
        $results = $this->permittimeline_model->listajax(
            $columns,
        ...
        foreach ($results as $r) {
            ...
            $rud = "";
            if ($this->permission->cp_read == true) {
                $rud .= anchor(site_url('permittimeline/read/' . fixzy_encoder($r['permit_timeline_id'])), '<button type="button" class="btn btn-info"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></button>') .
                    ' ';
            }
            if ($this->permission->cp_update == true) {
                $rud .= anchor(site_url('permittimeline/update/' . fixzy_encoder($r['permit_timeline_id'])), '<button type="button" class="btn btn-default"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>') .
                    ' ';
            }
            if ($this->permission->cp_delete == true) {
                $rud .= anchor(site_url('permittimeline/delete/' . fixzy_encoder($r['permit_timeline_id'])), '<button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></button>', 'onclick="javasciprt: return confirm(\'' . $this->lang->line('delete_alert') . '\')"');
            }
            array_push($data, [
                $i,
                $r['permit_timeline_permitid'],
                $r['permit_timeline_userid'],
                $r['permit_timeline_name'],
                $r['permit_timeline_desc'],
                $r['permit_timeline_status'],
                $r['permit_timeline_officialstatus'],
                $r['permit_timeline_created_at'],
                $r['permit_timeline_updated_at'],
                $r['permit_timeline_deleted_at'],
        ...
        echo json_encode(
            [
                ...
                "recordsTotal" => $this->permittimeline_model->recordsTotal()->recordstotal,
                "recordsFiltered" => $this->permittimeline_model->recordsFiltered($columns, $this->input->get('search')['value'])->recordsfiltered,
                'data' => $data
            ]
        );
    }
...
;
/* End of file Permittimeline.php */
/* Location: ./application/controllers/Permittimeline.php */
