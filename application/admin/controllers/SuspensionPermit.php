...
class SuspensionPermit extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->permission->showlist == true) {
            ...
            $data = array();//$this->Service_charges_model->get_all();
            /* $this->logQueries($this->config->item('dblog')); */
            $data = [
                'serviceCharges_data' => $data,
                'permission' => $this->permission,
            ];
            $this->content = 'suspensionPermit/list';
    ...
    function ajaxListSuspend()
    {
        $list = $this->Enforcement_permit_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) 
        {

            $url_permit_type = strtolower($field->permit_type_name_permit_typeid);

            $no++;
            $row = array();
            $row[] = $no;
            $row[] = $field->company_name_permit_companyid;
            $row[] = $field->permit_issuance_serialno;
            $row[] = $field->permit_type_desc;
            $row[] = date('d-m-Y',strtotime($field->permit_created_at));
            $row[] = '<a href="'.site_url("permitall/".$url_permit_type."/".fixzy_encoder($field->permit_id)).'"><button type="button" class="btn btn-info"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></button></a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Enforcement_permit_model->count_all(),
            "recordsFiltered" => $this->Enforcement_permit_model->count_filtered(),
            "data" => $data,
        );
