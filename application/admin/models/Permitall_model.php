...
//SEMENTARA
    public function adp161()
    {
        $this->db->select('*');
        $this->db->where('permit.permit_issuance_serialno', 'ADP000161');
        $this->db->where('permit_deleted_at');
        $this->db->from('permit');
...
/* update driver or vehicle status */
    ...
    public function get_read_pic($id)
    {
        $this->db->select("
    permit.*,
    permit_group.permit_group_name AS permit_group_name_permit_groupid,
...
// delete data
    ...
    public function get_count_permit($type)
    {
        if ($type == 'adp') {
            $group = [1];
        } elseif ($type == 'evdp') {
            ...
            $group = [3];
        } elseif ($type == 'cpu' || $type == 'vdgs' || $type == 'gpu' || $type == 'pbb') {
            $group = [5, 6, 7, 8];
        } else {
            $group = [9, 10, 11, 12, 13];
        }
        $this->db->select('count(*) as countmypermit', false);
        $this->db->where_in('permit.permit_typeid', $group);
        $this->db->where('permit.permit_issuance_serialno IS NOT NULL');
        $this->db->where('permit_deleted_at');
        $this->db->from('permit');
        ...
        //return $query->num_rows();
        $permit = $query->row();
        return $permit->countmypermit;

    }

        public function get_lastserialno($type)
    {
        if ($type == 'adp') {
            $group = [1];
        } elseif ($type == 'evdp') {
        ...
        $this->db->select('permit_issuance_serialno', false);
        $this->db->where_in('permit.permit_typeid', $group);
        $this->db->where('permit.permit_issuance_serialno IS NOT NULL');
        $this->db->where('permit_deleted_at');
        $this->db->order_by('permit_issuance_serialno','DESC');
        $this->db->limit(1);
        $this->db->from('permit');
        ...
        //return $query->num_rows();
        $permit = $query->row();
        return $permit->permit_issuance_serialno;

    }

    public function get_count_receiptno()
    {

        $this->db->select('*');
        $this->db->where('permit.permit_payment_invoiceno IS NOT NULL');
        $this->db->where('permit_deleted_at');
        $this->db->from('permit');
    ...
    public function get_lastinvoiceno()
    {

        $this->db->select('permit_payment_invoiceno');
        $this->db->where('permit.permit_payment_invoiceno IS NOT NULL');
        $this->db->where('permit.permit_payment_invoiceno <>','');
        $this->db->where('permit.permit_payment_invoiceno <>','foc');
        $this->db->where('permit.permit_payment_invoiceno <>','FOC');
        $this->db->where('permit_deleted_at');
        $this->db->order_by('permit_payment_invoiceno','DESC');
        $this->db->limit(1);
        $this->db->from('permit');

        $query = $this->db->get();
        $permit = $query->row();
        return $permit->permit_payment_invoiceno;
