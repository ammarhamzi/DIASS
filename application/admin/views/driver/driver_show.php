<style type="text/css">
    /* Tab styling for Enforcement History, Permits, Company History */
    .nav-tabs {
        padding-left: 15px;
        margin-bottom: 0;
        border: none;
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <ul class="nav nav-tabs" role="tablist">
                                    ...
                                    <li role="presentation" ><a href="#tab_companyhistory" aria-controls="tab_companyhistory" role="tab" data-toggle="tab">Company History</a></li>
                                </ul>
...
<?php
$start = 0;
if ($history_list && count($history_list) > 0) {
    foreach ($history_list as $r) {
        ...
        ?>
                                                              <tr>
                                                                ...
                                                                <td><?php echo rtrim($r->enforcements_main_period_suspension, '. '); ?></td>
...
if ($permission->cp_create == true) {
    ...
    // Empty state - ensure table structure is valid
    ?>
    <tr>
        <td colspan="7" class="text-center">No enforcement history found</td>
    </tr>
    <?php
}
?>
...
?>
                                    ...
                                    <div role="tabpanel" class="tab-pane" id="tab_companyhistory">
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <table id="mytable3" class="table" style="width: 100% !important">
                                                            <thead>
                                                                <tr>
                                                                    <th>#</th>
                                                                    <th><?php echo 'Company Name'; //$this->lang->line('user_username');     ?></th>
                                                                    <th><?php echo 'Changed By'; //$this->lang->line('user_email');     ?></th>
                                                                    <th><?php echo 'Date of Change'; //$this->lang->line('user_username');     ?></th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                            <?php
 if($companyhistory_list){
$noCount_h = 1;
foreach ($companyhistory_list as $cl) {
    ?>
                                                                <tr>
                                                                    <td><?=$noCount_h++;?></td>
                                                                    <td><?=$cl->company_name;?></td>
                                                                    <td><?=$cl->user_name;?></td>
                                                                    <td><?=$cl->drivercompanyhistory_created_at;?></td>
                                                                </tr>
                                                            <?php
}
}else{
...
?>
    ...
    <div class="modal" id="modal_history_offence_list" tabindex="-1" role="dialog" aria-labelledby="modalOffenceListLabel">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modalOffenceListLabel">Offence List</h4>
                </div>
...
<script>
    ...
    $(document).ready(function ()
    {
        ...
        $('#mytable').on('click','.btn_history_offence_list',function(){
            ...
            console.log('Button clicked, enc:', enc);

            // Force modal to show with proper configuration
            var $modal = $('#modal_history_offence_list');
            ...
            // Force modal to be visible (handle both Bootstrap 3 and 4/5)
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#span_offence_list').html('<div class="alert alert-danger">Error loading offence list: ' + error + '</div>');
                },
                complete : function()
                {
        ...
        try {
            ...
            console.error('DataTable initialization error on #mytable:', e);
            // Try without responsive if it fails
            try {
                var tables = $("#mytable").DataTable({
                    columnDefs:[
                        { targets: 'no-sort', orderable: false }
                ...
                console.error('DataTable initialization failed completely:', e2);
            }
        }
        ...
        var tables2 = $("#mytable3").DataTable({
            responsive: true,
            columnDefs:[
                { targets: 'no-sort', orderable: false }
            ]
        ...
        // Handle tab color switching for Enforcement History, Permits, and Company History tabs
        function updateTabColors() {
            console.log('Updating tab colors...');
            // Reset all tabs to gray
            $('.nav-tabs li').each(function() {
                $(this).removeClass('active');
                var link = $(this).find('a')[0];
                if (link) {
                    link.style.setProperty('background-color', '#ecf0f5', 'important');
                    link.style.setProperty('color', '#666', 'important');
                    link.style.setProperty('border-color', '#ddd', 'important');
                }
            });
            ...
            $('.nav-tabs li.active').each(function() {
                var link = $(this).find('a')[0];
                if (link) {
                    link.style.setProperty('background-color', '#3c8dbc', 'important');
                    link.style.setProperty('color', '#ffffff', 'important');
                    link.style.setProperty('border-color', '#3c8dbc', 'important');
                }
            });
        }
        
        // Listen to both click and shown events
        ...
        $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            console.log('Tab shown:', $(this).text());
            updateTabColors();
