...
//set the class variable.
    ...
    public function __construct()
    {
        ...
        parent::__construct();
                   ...
                   // Disabled HTTPS enforcement for local development
        ...
        if ($this->session->userdata('authentication') == "y" && !$this->session->userdata('islogin') && strtolower($this->router->fetch_class()) != "authentication") {
            ...
            if ($this->check_requiredinfo() == false && strtolower($this->router->fetch_class()) != "profile" && strtolower($this->router->fetch_class()) != "authentication") {
                ...
                if($this->session->userdata('islogin') == '1' && $this->session->userdata('groupid') == '2'){
                    redirect($this->config->item('client_url'));
                }
    ...
    public function layout($data = [], $setting = [], $show_menu = 1, $parentchildmenu = [])
    {
        ...
        if ($this->config->item('apps_type') == 'web') {
            ...
            if (isset($this->infopage)) {
                $template['infopage'] = $this->load->view($this->infopage, $data, true);
            }
    ...
    public function get_menu($showme = 1, $is_gen = 0)
    {
        ...
        $parentmenus     = $this->my_model->get_data('menu', '*', "menu_parentid = 0 and menu_isactive = 1 and menu_deleted_at IS NULL and menu_id in (select permission_menuid from permission where permission_usergroup = $groupid and (permission_inactive is null or permission_inactive = '')) order by menu_sort asc");
        ...
        if ($is_gen == 0) {
            if ($this->config->item('apps_type') == 'web') {
                if ($showme == 1) {
                    ...
                    if ($groupid == 14) {
                    return $this->load->view('foundation/exammenu', $menus, true);
                    }else{return $this->load->view('foundation/menu', $menus, true);}
                } else {
                    return null;
                }
    ...
    public function pic_detail($id){
    //$id = fixzy_decoder($id);
    $row         = $this->pic_model->get_by_permitid($id);
    if ($row) {
            $data = [
                'pic_id' => $row->pic_id,
                'pic_company_id' => $row->pic_company_id,
...
'permit_company_receipt' => $row->permit_company_receipt,
            ...
            $data_pic = $this->pic_detail($id);

            $data = array_merge($data_permit, $data_adppermit, $data_driver, $data_timeline, $data_files, $data_driverphoto, $data_examset, $data_extra, $data_pic);
...
'permit_company_receipt' => $row->permit_company_receipt,
            ...
            $data = array_merge($data_permit, $data_evdppermit, $data_driver, $data_timeline, $data_files, $data_driverphoto, $data_extra, $data_pic);
            return $data;
...
/**
 * Global AVP detail.
 * Shared all over system.
 *
 * @access  public
 * @param   int     permitid
 * @param   string  normal/raw
 * @return  array
 */
    public function avp_detail($id)
    {
        ...
        $data_permit = [
            ...
            'permit_payment_total' => standardmoney($row->permit_payment_total),
            'permit_payment_processedby' => $row->user_name_permit_payment_processedby,
            'permit_statusname' => $row->permit_status,
            'permit_status' => $row->permit_status_desc_permit_status,
            'permit_officialstatus' => $row->permit_officialstatus_name_permit_officialstatus,
            'permit_remark' => (!empty($row->permit_remark)?$row->permit_remark:"-"),
            'permit_docscheck_remark' => $row->permit_docscheck_remark,
...
/*        'avppermit_recent_avp_expirydate' => $avppermit->avppermit_recent_avp_expirydate,*/
                ...
                'avppermit_avpcategory' => $avppermit->avppermit_avpcategory,
            ];
        }
...
/*                        'vehicle_parkingarea_id' => $vehicle->parkingarea_name_vehicle_parkingarea_id, */
                ...
                'vehicle_engine_capacity_id' => $vehicle->vehicle_engine_capacity,
...
//        }
        ...
        $data = array_merge($data_permit, $data_avppermit, $data_vehicle, $data_timeline, $data_files, $data_inspection, $data_extra, $data_pic);
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_evppermit, $data_vehicle, $data_timeline, $data_files, $data_inspection, $data_extra, $data_pic);
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_pbbpermit, $data_driver, $data_timeline, $data_files, $data_driverphoto, $data_extra, $data_pic);
            return $data;
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_shpermit, $data_vehicle, $data_timeline, $data_files, $data_extra, $data_pic);
            return $data;
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_vdgspermit, $data_driver, $data_timeline, $data_files, $data_driverphoto, $data_extra, $data_pic);
            return $data;
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_cspermit, $data_vehicle, $data_timeline, $data_files, $data_extra, $data_pic);
            return $data;
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_gpupermit, $data_driver, $data_timeline, $data_files, $data_driverphoto, $data_extra, $data_pic);
            return $data;
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_pcapermit, $data_driver, $data_timeline, $data_files, $data_driverphoto, $data_extra, $data_pic);
            return $data;
...
'permit_company_receipt' => $row->permit_company_receipt,
        ...
        if ($wippermit) {
            $data_wippermit = [
                ...
                'wippermit_ownverified_date' => $wippermit->wippermit_ownverified_date,
                'wippermit_result' => $wippermit->wippermit_result,
                'wippermit_result_inspector_id' => (!empty($wippermit->inspector_name)?$wippermit->inspector_name:"-"),
                'wippermit_inspection_date' => (!empty($wippermit->wippermit_inspection_date)?$wippermit->wippermit_inspection_date:"-"),
                'wippermit_retest_result' => $wippermit->wippermit_retest_result,
                'wippermit_retest_result_inspector_id' => (!empty($wippermit->wippermit_retest_result_inspector_id)?$wippermit->wippermit_retest_result_inspector_id:"-"),
                'wippermit_retest_inspection_date' => (!empty($wippermit->wippermit_retest_inspection_date)?$wippermit->wippermit_retest_inspection_date:"-"),
                'wippermit_managerverified_id' => $wippermit->wippermit_managerverified_id,
                'engineer_name' => $wippermit->engineer_name,
                'wippermit_managerverified_date' => $wippermit->wippermit_managerverified_date,
...
/*        'wippermit_recent_wip_expirydate' => $wippermit->wippermit_recent_wip_expirydate,*/
                ...
                'wippermit_fireext_serialno_mtw' => (!empty($wippermit->wippermit_fireext_serialno_mtw)?$wippermit->wippermit_fireext_serialno_mtw:"-"),
                'wippermit_fireext_expirydate_mtw' => (!empty($wippermit->wippermit_fireext_expirydate_mtw)?$wippermit->wippermit_fireext_expirydate_mtw:"-"),
                'wippermit_tyre_manufacturingdate_mtw' => (!empty($wippermit->wippermit_tyre_manufacturingdate_mtw)?$wippermit->wippermit_tyre_manufacturingdate_mtw:"-"),
                'wippermit_smokecondition_mtw' => (!empty($wippermit->wippermit_smokecondition_mtw)?$wippermit->wippermit_smokecondition_mtw:"-"),

                'wippermit_inspection_location' => $wippermit->wippermit_inspection_location,
                /*'wippermit_wipcategory' => $wippermit->wippermit_wipcategory,*/
                    'wippermit_entrypurpose' => $wippermit->wippermit_entrypurpose,
                    'wippermit_entrypost' => $wippermit->wippermit_entrypost,
                    'wippermit_exitpost' => $wippermit->wippermit_exitpost,
                    'wippermit_steerman_name' => $wippermit->wippermit_steerman_name,
...
$data_pic = $this->pic_detail($id);
        ...
        $data = array_merge($data_permit, $data_wippermit, $data_vehicle, $data_timeline, $data_files, $data_inspection, $data_extra, $data_pic);

        return $data;

    }

    public function pic_list($companyid, $getdata)
    {
        $adminlist = $this->user_model->get_pic($companyid);
...
$data_pic = $this->pic_detail($id);
        ...
        $data = array_merge($data_permit, $data_shinspermit, $data_vehicle, $data_timeline, $data_files, $data_inspection, $data_extra, $data_pic);
...
$data_pic = $this->pic_detail($id);
            ...
            $data = array_merge($data_permit, $data_wipbriefingpermit, $data_vehicle, $data_timeline, $data_files, $data_extra, $data_pic);
            return $data;
